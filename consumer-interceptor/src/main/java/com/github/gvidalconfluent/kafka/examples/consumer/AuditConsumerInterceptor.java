...
import com.github.gvidalconfluent.kafka.examples.consumer.internal.Time;
import com.github.gvidalconfluent.kafka.examples.consumer.internal.Utils;
...
public class AuditConsumerInterceptor<K, V> implements ConsumerInterceptor<K, V> {
    ...
    private String getJsonTrackingMessage(ConsumerRecord<K, V> record) {
        //Get the trackingOnSendEpochTime
        Header lastEpoch = record.headers().lastHeader("trackingOnSendEpochTime");
        Bytes timestamp = Bytes.wrap(lastEpoch.value());
        long longTS = Utils.bytesToLong(timestamp);
        long duration = Time.SYSTEM.milliseconds() - longTS;
